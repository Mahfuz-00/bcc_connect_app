import 'dart:convert';
import 'package:bcc_connect_app/API%20Model%20and%20Service%20(Regions)/regionmodels.dart';
    ...
    try {
      ...
      if (e is FormatException) {
        final response = await http
            .get(Uri.parse('https://bcc.touchandsolve.com/api/division'));
  ...
  Future<List<Union>> fetchUnions(String upazilaId) async {
    try{
      ...
      if (e is FormatException) {
        ...
        final response = await http
            .get(Uri.parse('https://bcc.touchandsolve.com/api/division'));
        print('Failed to load Unions: $e, ${response.statusCode}');
      }
      return [];
    }
  }
   ...
   try{
     ...
     if (response.statusCode == 200) {
       ...
       if (data != null && data.containsKey('records')) {
         final List<dynamic> records = data['records'];
         print('Record:: $records');
         final List<NTTNProvider> NTTNs =  records.map((record) => NTTNProvider.fromJson(record)).toList();
         print(NTTNs);
         return NTTNs;
       } else {
         throw Exception(
